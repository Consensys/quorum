...
type PermissionContractService interface {
	...
	ApproveBlacklistedNodeRecovery(_orgId string, _enodeId string, _ip [32]byte, _port uint16, _raftport uint16, _url string) (*types.Transaction, error)
	StartBlacklistedNodeRecovery(_orgId string, _enodeId string, _ip [32]byte, _port uint16, _raftport uint16, _url string) (*types.Transaction, error)
	...
	AddNode(_orgId string, _enodeId string, _ip [32]byte, _port uint16, _raftport uint16, _url string) (*types.Transaction, error)
	UpdateNodeStatus(_orgId string, _enodeId string, _ip [32]byte, _port uint16, _raftport uint16, _url string, _action *big.Int) (*types.Transaction, error)
	...
	ApproveOrg(_orgId string, _enodeId string, _ip [32]byte, _port uint16, _raftport uint16, _account common.Address, _url string) (*types.Transaction, error)
	AddSubOrg(_pOrgId string, _orgId string, _enodeId string, _ip [32]byte, _port uint16, _raftport uint16, _url string) (*types.Transaction, error)
	AddOrg(_orgId string, _enodeId string, _ip [32]byte, _port uint16, _raftport uint16, _account common.Address, _url string) (*types.Transaction, error)
...
func NewPermissionContractService(ethClnt bind.ContractBackend, eeaFlag bool, key *ecdsa.PrivateKey,
	...
	if eeaFlag {
		return &PermissionContractEea{ethClnt: ethClnt, eeaFlag: eeaFlag, key: key, permConfig: permConfig}
	}
	return &PermissionContractBasic{ethClnt: ethClnt, eeaFlag: eeaFlag, key: key, permConfig: permConfig}
}
...
func NewPermissionContractServiceForApi(p *PermissionCtrl, frmAcct accounts.Account, transactOpts *bind.TransactOpts, gasLimit uint64, gasPrice *big.Int) PermissionContractService {
	...
	return &PermissionContractBasic{permInterfSession: ps, eeaFlag: p.eeaFlag, permConfig: p.permConfig}
