...
type PermissionCtrl struct {
	...
	permSrvc *PermissionContractService
...
func (p *PermissionCtrl) AsyncStart() {
	...
	p.Eth = ethereum
	p.permSrvc = &PermissionContractService{EthClnt: p.EthClnt, EeaFlag: p.eeaFlag, Key: p.Key, PermConfig: p.PermConfig}
}
...
func (p *PermissionCtrl) ManageOrgPermissionsE() error {
	...
	if _, err := p.permSrvc.PermOrgE.EeaOrgManagerFilterer.WatchOrgPendingApproval(opts, chPendingApproval); err != nil {
		return fmt.Errorf("failed WatchNodePendingApproval: %v", err)
	}

	if _, err := p.permSrvc.PermOrgE.EeaOrgManagerFilterer.WatchOrgApproved(opts, chOrgApproved); err != nil {
		return fmt.Errorf("failed WatchNodePendingApproval: %v", err)
	}

	if _, err := p.permSrvc.PermOrgE.EeaOrgManagerFilterer.WatchOrgSuspended(opts, chOrgSuspended); err != nil {
		return fmt.Errorf("failed WatchNodePendingApproval: %v", err)
	}

	if _, err := p.permSrvc.PermOrgE.EeaOrgManagerFilterer.WatchOrgSuspensionRevoked(opts, chOrgReactivated); err != nil {
		return fmt.Errorf("failed WatchNodePendingApproval: %v", err)
	}
...
func (p *PermissionCtrl) ManageOrgPermissionsBasic() error {
	...
	if _, err := p.permSrvc.PermOrg.OrgManagerFilterer.WatchOrgPendingApproval(opts, chPendingApproval); err != nil {
		return fmt.Errorf("failed WatchNodePendingApproval: %v", err)
	}

	if _, err := p.permSrvc.PermOrg.OrgManagerFilterer.WatchOrgApproved(opts, chOrgApproved); err != nil {
		return fmt.Errorf("failed WatchNodePendingApproval: %v", err)
	}

	if _, err := p.permSrvc.PermOrg.OrgManagerFilterer.WatchOrgSuspended(opts, chOrgSuspended); err != nil {
		return fmt.Errorf("failed WatchNodePendingApproval: %v", err)
	}

	if _, err := p.permSrvc.PermOrg.OrgManagerFilterer.WatchOrgSuspensionRevoked(opts, chOrgReactivated); err != nil {
		return fmt.Errorf("failed WatchNodePendingApproval: %v", err)
	}
...
func (p *PermissionCtrl) ManageNodePermissionsBasic() error {
	...
	if _, err := p.permSrvc.PermNode.NodeManagerFilterer.WatchNodeApproved(opts, chNodeApproved); err != nil {
		return fmt.Errorf("failed WatchNodeApproved: %v", err)
	}

	if _, err := p.permSrvc.PermNode.NodeManagerFilterer.WatchNodeProposed(opts, chNodeProposed); err != nil {
		return fmt.Errorf("failed WatchNodeProposed: %v", err)
	}

	if _, err := p.permSrvc.PermNode.NodeManagerFilterer.WatchNodeDeactivated(opts, chNodeDeactivated); err != nil {
		return fmt.Errorf("failed NodeDeactivated: %v", err)
	}
	if _, err := p.permSrvc.PermNode.NodeManagerFilterer.WatchNodeActivated(opts, chNodeActivated); err != nil {
		return fmt.Errorf("failed WatchNodeActivated: %v", err)
	}

	if _, err := p.permSrvc.PermNode.NodeManagerFilterer.WatchNodeBlacklisted(opts, chNodeBlacklisted); err != nil {
		return fmt.Errorf("failed NodeBlacklisting: %v", err)
	}

	if _, err := p.permSrvc.PermNode.NodeManagerFilterer.WatchNodeRecoveryInitiated(opts, chNodeRecoveryInit); err != nil {
		return fmt.Errorf("failed NodeRecoveryInitiated: %v", err)
	}

	if _, err := p.permSrvc.PermNode.NodeManagerFilterer.WatchNodeRecoveryCompleted(opts, chNodeRecoveryDone); err != nil {
...
func (p *PermissionCtrl) ManageNodePermissionsE() error {
	...
	if _, err := p.permSrvc.PermNodeE.EeaNodeManagerFilterer.WatchNodeApproved(opts, chNodeApproved); err != nil {
		return fmt.Errorf("failed WatchNodeApproved: %v", err)
	}

	if _, err := p.permSrvc.PermNodeE.EeaNodeManagerFilterer.WatchNodeProposed(opts, chNodeProposed); err != nil {
		return fmt.Errorf("failed WatchNodeProposed: %v", err)
	}

	if _, err := p.permSrvc.PermNodeE.EeaNodeManagerFilterer.WatchNodeDeactivated(opts, chNodeDeactivated); err != nil {
		return fmt.Errorf("failed NodeDeactivated: %v", err)
	}
	if _, err := p.permSrvc.PermNodeE.EeaNodeManagerFilterer.WatchNodeActivated(opts, chNodeActivated); err != nil {
		return fmt.Errorf("failed WatchNodeActivated: %v", err)
	}

	if _, err := p.permSrvc.PermNodeE.EeaNodeManagerFilterer.WatchNodeBlacklisted(opts, chNodeBlacklisted); err != nil {
		return fmt.Errorf("failed NodeBlacklisting: %v", err)
	}

	if _, err := p.permSrvc.PermNodeE.EeaNodeManagerFilterer.WatchNodeRecoveryInitiated(opts, chNodeRecoveryInit); err != nil {
		return fmt.Errorf("failed NodeRecoveryInitiated: %v", err)
	}

	if _, err := p.permSrvc.PermNodeE.EeaNodeManagerFilterer.WatchNodeRecoveryCompleted(opts, chNodeRecoveryDone); err != nil {
		return fmt.Errorf("failed NodeRecoveryCompleted: %v", err)
	}
...
func (p *PermissionCtrl) ManageAccountPermissionsE() error {
	...
	if _, err := p.permSrvc.PermAcctE.EeaAcctManagerFilterer.WatchAccountAccessModified(opts, chAccessModified); err != nil {
		return fmt.Errorf("failed AccountAccessModified: %v", err)
	}

	if _, err := p.permSrvc.PermAcctE.EeaAcctManagerFilterer.WatchAccountAccessRevoked(opts, chAccessRevoked); err != nil {
		return fmt.Errorf("failed AccountAccessRevoked: %v", err)
	}

	if _, err := p.permSrvc.PermAcctE.EeaAcctManagerFilterer.WatchAccountStatusChanged(opts, chStatusChanged); err != nil {
...
func (p *PermissionCtrl) ManageAccountPermissionsBasic() error {
	...
	if _, err := p.permSrvc.PermAcct.AcctManagerFilterer.WatchAccountAccessModified(opts, chAccessModified); err != nil {
		return fmt.Errorf("failed AccountAccessModified: %v", err)
	}

	if _, err := p.permSrvc.PermAcct.AcctManagerFilterer.WatchAccountAccessRevoked(opts, chAccessRevoked); err != nil {
		return fmt.Errorf("failed AccountAccessRevoked: %v", err)
	}

	if _, err := p.permSrvc.PermAcct.AcctManagerFilterer.WatchAccountStatusChanged(opts, chStatusChanged); err != nil {
		return fmt.Errorf("failed AccountStatusChanged: %v", err)
	}
...
func (p *PermissionCtrl) PopulateInitPermissions(orgCacheSize, roleCacheSize, nodeCacheSize, accountCacheSize int) error {
	...
	networkInitialized, err := p.permSrvc.GetNetworkBootStatus()
	if err != nil {
...
func (p *PermissionCtrl) BootupNetwork() error {
	if _, err := p.permSrvc.SetPolicy(p.PermConfig.NwAdminOrg, p.PermConfig.NwAdminRole, p.PermConfig.OrgAdminRole); err != nil {
		log.Error("bootupNetwork SetPolicy failed", "err", err)
		return err
	}
	if _, err := p.permSrvc.Init(p.PermConfig.SubOrgBreadth, p.PermConfig.SubOrgDepth); err != nil {
...
func (p *PermissionCtrl) PopulateAccountsFromContract() error {
	if numberOfRoles, err := p.permSrvc.GetNumberOfAccounts(); err == nil {
		iOrgNum := numberOfRoles.Uint64()
		for k := uint64(0); k < iOrgNum; k++ {
			if addr, org, role, status, orgAdmin, err := p.permSrvc.GetAccountDetailsFromIndex(big.NewInt(int64(k))); err == nil {
...
func (p *PermissionCtrl) PopulateRolesFromContract() error {
	if numberOfRoles, err := p.permSrvc.GetNumberOfRoles(); err == nil {
		iOrgNum := numberOfRoles.Uint64()
		for k := uint64(0); k < iOrgNum; k++ {
			if roleStruct, err := p.permSrvc.GetRoleDetailsFromIndex(big.NewInt(int64(k))); err == nil {
...
func (p *PermissionCtrl) PopulateNodesFromContract() error {
	if numberOfNodes, err := p.permSrvc.GetNumberOfNodes(); err == nil {
		iOrgNum := numberOfNodes.Uint64()
		for k := uint64(0); k < iOrgNum; k++ {
			if nodeStruct, err := p.permSrvc.GetNodeDetailsFromIndex(big.NewInt(int64(k))); err == nil {
...
func (p *PermissionCtrl) PopulateOrgsFromContract() error {
	...
	if numberOfOrgs, err := p.permSrvc.GetNumberOfOrgs(); err == nil {
		iOrgNum := numberOfOrgs.Uint64()
		for k := uint64(0); k < iOrgNum; k++ {
			if orgId, porgId, ultParent, level, status, err := p.permSrvc.GetOrgInfo(big.NewInt(int64(k))); err == nil {
...
func (p *PermissionCtrl) PopulateStaticNodesToContract() error {
	...
	for _, node := range nodes {
		...
		_, err := p.permSrvc.AddAdminNode(enodeId, ip, port, raftPort)
		if err != nil {
...
func (p *PermissionCtrl) PopulateInitAccountAccess() error {
	for _, a := range p.PermConfig.Accounts {
		_, er := p.permSrvc.AddAdminAccount(a)
		if er != nil {
...
func (p *PermissionCtrl) UpdateNetworkStatus() error {
	_, err := p.permSrvc.UpdateNetworkBootStatus()
	if err != nil {
...
func (p *PermissionCtrl) ManageRolePermissionsE() error {
	...
	if _, err := p.permSrvc.PermRoleE.EeaRoleManagerFilterer.WatchRoleCreated(opts, chRoleCreated); err != nil {
		return fmt.Errorf("failed WatchRoleCreated: %v", err)
	}

	if _, err := p.permSrvc.PermRoleE.EeaRoleManagerFilterer.WatchRoleRevoked(opts, chRoleRevoked); err != nil {
...
func (p *PermissionCtrl) ManageRolePermissionsBasic() error {
	...
	if _, err := p.permSrvc.PermRole.RoleManagerFilterer.WatchRoleCreated(opts, chRoleCreated); err != nil {
		return fmt.Errorf("failed WatchRoleCreated: %v", err)
	}

	if _, err := p.permSrvc.PermRole.RoleManagerFilterer.WatchRoleRevoked(opts, chRoleRevoked); err != nil {
		return fmt.Errorf("failed WatchRoleRemoved: %v", err)
	}
...
func (p *PermissionCtrl) PopulateAccountToCache(acctId common.Address) (*types.AccountInfo, error) {
	account, orgId, roleId, status, isAdmin, err := p.permSrvc.GetAccountDetails(acctId)
	if err != nil {
		return nil, err
	}
...
func (p *PermissionCtrl) PopulateOrgToCache(orgId string) (*types.OrgInfo, error) {
	org, parentOrgId, ultimateParentId, orgLevel, orgStatus, err := p.permSrvc.GetOrgDetails(orgId)
	if err != nil {
		return nil, err
	}
	...
	subOrgIndexes, err := p.permSrvc.GetSubOrgIndexes(orgId)
	if err != nil {
		return nil, err
	}
	...
	for _, s := range subOrgIndexes {
		subOrgId, _, _, _, _, err := p.permSrvc.GetOrgInfo(s)
...
func (p *PermissionCtrl) PopulateRoleToCache(roleKey *types.RoleKey) (*types.RoleInfo, error) {
	roleDetails, err := p.permSrvc.GetRoleDetails(roleKey.RoleId, roleKey.OrgId)
...
func (p *PermissionCtrl) PopulateNodeCache(url string) (*types.NodeInfo, error) {
	nodeDetails, err := p.permSrvc.GetNodeDetails(url)
	if err != nil {
		return nil, err
	}
