...
func (abi ABI) Unpack(v interface{}, name string, output []byte) error {
	...
	if len(method.Outputs) <= 0 {
		...
		return set(valueOf.Elem(), reflect.ValueOf(marshalledValue), method.Outputs[0])
	}
	...
	case reflect.Struct:
		for i := 0; i < len(method.Outputs); i++ {
			...
			for j := 0; j < typ.NumField(); j++ {
				...
				if field.Name == strings.ToUpper(method.Outputs[i].Name[:1])+method.Outputs[i].Name[1:] {
					err = set(value.Field(j), reflectValue, method.Outputs[i])
					if err != nil {
						return err
					}
				}
		...
		return nil
	case reflect.Slice:
		if !typ.AssignableTo(r_interSlice) {
