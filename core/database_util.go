...
func GetBlockReceipts(db DatabaseReader, hash common.Hash, number uint64, isPrivate bool) types.Receipts {
	var prefix []byte
	if isPrivate {
		prefix = privateblockReceiptsPrefix
	} else {
		prefix = blockReceiptsPrefix
	}

	data, _ := db.Get(append(append(prefix, encodeBlockNumber(number)...), hash[:]...))
	if len(data) == 0 {
		return nil
	}
...
// its added positional metadata.
func GetReceipt(db DatabaseReader, hash common.Hash, isPrivate bool) (*types.Receipt, common.Hash, uint64, uint64) {
	// Retrieve the lookup metadata and resolve the receipt from the receipts
	blockHash, blockNumber, receiptIndex := GetTxLookupEntry(db, hash)

	if blockHash != (common.Hash{}) {
		receipts := GetBlockReceipts(db, blockHash, blockNumber, isPrivate)
...
func WriteBlockReceipts(db ethdb.Putter, hash common.Hash, number uint64, receipts types.Receipts, isPrivate bool) error {
	var prefix []byte
	if isPrivate {
		prefix = privateblockReceiptsPrefix
	} else {
	...
	key := append(append(prefix, encodeBlockNumber(number)...), hash.Bytes()...)
...
func DeleteBlockReceipts(db DatabaseDeleter, hash common.Hash, number uint64) {
	...
	db.Delete(append(append(privateblockReceiptsPrefix, encodeBlockNumber(number)...), hash.Bytes()...))
}
