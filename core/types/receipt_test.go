...
func TestMPSReceiptDecoding(t *testing.T) {
	...
	innerReceipt.Bloom = CreateBloom(Receipts{innerReceipt})
	receipt := &Receipt{
	...
	receipt.PSIToReceipt = make(map[PrivateStateIdentifier]*Receipt)
	receipt.PSIToReceipt["psi1"] = innerReceipt
	...
	if len(dec.PSIToReceipt) != len(receipt.PSIToReceipt) {
		t.Fatalf("Receipt log number mismatch, want %v, have %v", len(receipt.PSIToReceipt), len(dec.PSIToReceipt))
	}

	for psi, decPsiReceipt := range dec.PSIToReceipt {
		wantedPsiReceipt := receipt.PSIToReceipt[psi]

		// Check whether all psi consensus fields are correct.
		if decPsiReceipt.Status != wantedPsiReceipt.Status {
			t.Fatalf("Receipt status mismatch, want %v, have %v", wantedPsiReceipt.Status, decPsiReceipt.Status)
		}
		if decPsiReceipt.CumulativeGasUsed != wantedPsiReceipt.CumulativeGasUsed {
			t.Fatalf("Receipt CumulativeGasUsed mismatch, want %v, have %v", wantedPsiReceipt.CumulativeGasUsed, decPsiReceipt.CumulativeGasUsed)
		}
		if decPsiReceipt.Bloom != wantedPsiReceipt.Bloom {
			t.Fatalf("Bloom data mismatch, want %v, have %v", wantedPsiReceipt.Bloom, decPsiReceipt.Bloom)
		}
		if len(decPsiReceipt.Logs) != len(wantedPsiReceipt.Logs) {
			t.Fatalf("Receipt log number mismatch, want %v, have %v", len(wantedPsiReceipt.Logs), len(decPsiReceipt.Logs))
		}
	}
}
...
func encodeAsStoredMPSReceiptRLP(want *Receipt) ([]byte, error) {
	stored := &storedMPSReceiptRLP{
		...
		CumulativeGasUsed: want.CumulativeGasUsed,
		Logs:              make([]*LogForStorage, len(want.Logs)),
		PSIToReceipt:      make([]storedPSIToReceiptMapEntry, len(want.PSIToReceipt)),
	}
	for i, log := range want.Logs {
		stored.Logs[i] = (*LogForStorage)(log)
	}
	idx := 0
	for key, val := range want.PSIToReceipt {
		...
		stored.PSIToReceipt[idx] = storedPSIToReceiptMapEntry{Key: key, Value: rec}
		idx++
	}
	return rlp.EncodeToBytes(stored)
}
...
func TestDeriveFieldsMPS(t *testing.T) {
	// Create a few transactions to have receipts for
	t1 := NewContractCreation(1, big.NewInt(1), 1, big.NewInt(1), nil)
	t1.SetPrivate()
	t2 := NewTransaction(2, common.HexToAddress("0x2"), big.NewInt(2), 2, big.NewInt(2), nil)
	t2.SetPrivate()
	txs := Transactions{
		t1,
		t2,
	...
	outerReceipt := innerReceipt
	outerReceipt.PSIToReceipt = make(map[PrivateStateIdentifier]*Receipt)
	outerReceipt.PSIToReceipt["psi1"] = innerReceipt
	// Create the corresponding receipts
	receipts := Receipts{
		&Receipt{
			Status:            ReceiptStatusFailed,
	...
	for i := range receipts {
		...
		for psi, privateReceipt := range receipts[i].PSIToReceipt {
			if psi != "psi1" {
				t.Errorf("incorrect psi")
			}
			if !reflect.DeepEqual(privateReceipt, innerReceipt) {
				t.Errorf("psi receipt is not equal to inner receipt")
			}
		}
	}
}
