...
func TestDAOForkRangeExtradata(t *testing.T) {
	...
	proBc, _ := NewBlockChain(proDb, nil, nil, &proConf, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer proBc.Stop()
	...
	conBc, _ := NewBlockChain(conDb, nil, nil, &conConf, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer conBc.Stop()
	...
	for i := int64(0); i < params.DAOForkExtraRange.Int64(); i++ {
		...
		db = rawdb.NewMemoryDatabase()
		gspec.MustCommit(db)
		bc, _ := NewBlockChain(db, nil, nil, &conConf, ethash.NewFaker(), vm.Config{}, nil, nil)
		defer bc.Stop()
		...
		db = rawdb.NewMemoryDatabase()
		gspec.MustCommit(db)
		bc, _ = NewBlockChain(db, nil, nil, &proConf, ethash.NewFaker(), vm.Config{}, nil, nil)
		defer bc.Stop()
