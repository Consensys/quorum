...
func TestSimulateExecution_whenTypicalMessageCall(t *testing.T) {
	assert := assert.New(t)
	privateTxArgs.PrivateStateValidation = false
	...
	assert.NoError(err, "simulate execution")
	assert.Contains(affectedCACreationTxHashes, arbitrarySimpleStorageContractEncryptedPayloadHash, "affected contracts")
	assert.Equal(common.Hash{}, merkleRoot, "no private state validation")
}
