...
type ManagementContractFacade interface {
	...
	Deploy(args *bind.TransactOpts, toExtend common.Address, voters []common.Address, recipientHash string) (*types.Transaction, error)
...
func (facade EthclientManagementContractFacade) Deploy(args *bind.TransactOpts, toExtend common.Address, voters []common.Address, recipientHash string) (*types.Transaction, error) {
	_, tx, _, err := extensionContracts.DeployContractExtender(args, facade.client, toExtend, voters, recipientHash)
	return tx, err
}
